...
// Schema for extracted dimensional pricing product
const ExtractedPricingEntrySchema = z.object({
  widthMin: z.number(),
  widthMax: z.number(),
  lengthMin: z.number(),
  lengthMax: z.number(),
...
const ExtractedDimensionalProductSchema = z.object({
  name: z.string(),
  manufacturer: z.string(),
  category: z.string().nullable().optional(),
  description: z.string().nullable().optional(),
  unit: z.string().default("each"),
  widthLabel: z.string().nullable().optional(), // e.g., "WIDTH", "Height"
  lengthLabel: z.string().nullable().optional(), // e.g., "HEIGHT", "Width" 
  widthUnit: z.string().default("inches"), // inches, feet, etc.
  lengthUnit: z.string().default("inches"),
  minWidth: z.number().nullable().optional(),
  maxWidth: z.number().nullable().optional(),
  minLength: z.number().nullable().optional(),
  maxLength: z.number().nullable().optional(),
  pricingEntries: z.array(ExtractedPricingEntrySchema),
  specialNotes: z.string().nullable().optional(),
  confidence: z.number().min(0).max(1).optional(),
});

export type ExtractedPricingEntry = z.infer<typeof ExtractedPricingEntrySchema>;
export type ExtractedDimensionalProduct = z.infer<typeof ExtractedDimensionalProductSchema>;
...
// Extract dimensional product data from PDF
export async function extractDimensionalProductFromPDF(pdfBuffer: Buffer): Promise<ExtractedDimensionalProduct | null> {
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are an expert at extracting product pricing data from manufacturer price sheets.

CRITICAL INSTRUCTIONS:
1. Extract the COMPLETE product name from the PDF header/title
2. Extract the manufacturer name
3. Identify ALL width and length/height dimension bands in the pricing table
4. Extract EVERY price from the pricing grid accurately
5. Convert ALL dimensions to INCHES (12" per foot)
6. Detect the unit labels used (WIDTH, HEIGHT, etc.)
7. Note any special instructions, constraints, or add-on pricing

PRICING TABLE FORMAT:
- Tables typically have WIDTH values as columns (across the top)
- HEIGHT/LENGTH values as rows (down the left side)
- Each cell contains the price for that width×height combination

DIMENSION CONVERSION:
- If dimensions show "7'" or "8'" those are FEET - multiply by 12 to get inches
- If dimensions show "84" or "96" those are already INCHES
- Example: 7' = 84 inches, 20' = 240 inches

Return JSON in this EXACT format:
{
  "name": "Full product name from PDF title",
  "manufacturer": "Manufacturer name",
  "category": "Product category if mentioned (e.g., 'Solar Screens', 'Pergolas')" or null,
  "description": "Any product description or special notes" or null,
  "unit": "each",
  "widthLabel": "The label used for width dimension (e.g., 'WIDTH', 'Height')" or null,
  "lengthLabel": "The label used for length dimension (e.g., 'HEIGHT', 'Width')" or null,
  "widthUnit": "inches",
  "lengthUnit": "inches",
  "minWidth": minimum width in inches as number or null,
  "maxWidth": maximum width in inches as number or null,
  "minLength": minimum length in inches as number or null,
  "maxLength": maximum length in inches as number or null,
  "pricingEntries": [
    {
      "widthMin": 48,
      "widthMax": 59,
  ...
  "specialNotes": "Any special pricing notes, add-ons, or constraints" or null,
  "confidence": 0.95
}

VALIDATION RULES:
- All dimensions MUST be in inches
- widthMax must be greater than widthMin
- lengthMax must be greater than lengthMin  
- Prices must be positive numbers
- Include ALL pricing entries from the table - DO NOT skip any rows or columns
- Confidence should reflect extraction accuracy (0.0 to 1.0)`
        },
        {
          role: "user",
          content: [
            {
              type: "text",
              text: "Extract the dimensional product pricing data from this manufacturer price sheet. Be thorough and extract EVERY dimension band and price."
            },
            ...imageContent
          ],
        },
      ...
      response_format: { type: "json_object" },
      max_completion_tokens: 16000, // Large token limit for complete pricing tables
    });
    ...
    const extracted = ExtractedDimensionalProductSchema.parse(parsedContent);
    
    // Validate pricing entries
    for (const entry of extracted.pricingEntries) {
      if (entry.widthMax <= entry.widthMin) {
        throw new Error(`Invalid width range: ${entry.widthMin} to ${entry.widthMax}`);
      }
      if (entry.lengthMax <= entry.lengthMin) {
        throw new Error(`Invalid length range: ${entry.lengthMin} to ${entry.lengthMax}`);
      }
      if (entry.price <= 0) {
        throw new Error(`Invalid price: ${entry.price}`);
      }
    }
    
    console.log(`✅ Successfully extracted ${extracted.pricingEntries.length} pricing entries`);
    return extracted;
    ...
    if (error instanceof z.ZodError) {
      ...
      console.error('📋 Product data validation failed:', {
        message: 'AI response did not match expected product structure',
        validationIssues: validationIssues,
      });
    } else {
      console.error('Error extracting dimensional product from PDF:', error);
    }
    return null;
  }
}
