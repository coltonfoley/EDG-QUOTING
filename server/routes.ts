...
import { extractQuoteDataFromImages, extractQuoteDataFromPDF, extractProductsFromManufacturerPDF } from "./openai";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Extract products from manufacturer price list PDF
  app.post("/api/products/import-from-pdf", isAuthenticated, rateLimitPDFProcessing, upload.single('pdf'), async (req: any, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ 
          message: "No PDF file uploaded",
      ...
      console.log(`📄 Extracting products from PDF: ${file.originalname} (${(file.size / 1024 / 1024).toFixed(2)}MB)`);

      // Extract products using OpenAI
      const extractedData = await extractProductsFromManufacturerPDF(file.buffer);
      
      if (!extractedData || !extractedData.products || extractedData.products.length === 0) {
        return res.status(400).json({ 
          message: "Could not extract products from PDF. Please ensure the document contains a valid price list with products and prices.",
          success: false 
        });
      }

      // Get all existing manufacturers from database
      const existingProducts = await db.selectDistinct({ manufacturer: products.manufacturer })
        .from(products)
        .orderBy(products.manufacturer);
      
      const existingManufacturers = existingProducts.map(p => p.manufacturer.toLowerCase());
      const extractedManufacturer = extractedData.manufacturer.toLowerCase();
      
      // Check if manufacturer exists in system
      const manufacturerExists = existingManufacturers.includes(extractedManufacturer);

      console.log(`✅ Extracted ${extractedData.products.length} products from ${extractedData.manufacturer} (${manufacturerExists ? 'existing' : 'new'} manufacturer)`);
      ...
      res.status(200).json({
        ...
        manufacturerExists,
        existingManufacturers: existingProducts.map(p => p.manufacturer),
        products: extractedData.products,
        message: `Successfully extracted ${extractedData.products.length} products from ${extractedData.manufacturer}`
      });

    } catch (error: any) {
      console.error("PDF product extraction error:", error);
      ...
      return res.status(500).json({ 
        message: `PDF processing failed: ${error.message || 'Unexpected error occurred'}`,
        success: false 
      });
    }
  });

  // Bulk import products from extracted data
  app.post("/api/products/bulk-import", isAuthenticated, async (req, res) => {
    try {
      ...
      const validatedData = bulkImportSchema.safeParse(req.body);
      if (!validatedData.success) {
        return res.status(400).json({ 
      ...
      const { manufacturer, products: productsToImport } = validatedData.data;
      ...
      console.log(`✅ Bulk imported ${importedProducts.length} products for ${manufacturer}`);
      ...
      res.status(500).json({ 
        message: `Failed to import products: ${error.message || 'Unexpected error'}`,
        success: false 
      });
    }
  });
