...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // PDF Product Import endpoint (AI-powered extraction)
  app.post('/api/admin/import-pdf-product', isAuthenticated, rateLimitPDFProcessing, upload.single('pdf'), async (req: any, res) => {
    try {
      const currentUser = await storage.getUser(req.user?.id);
      if (currentUser?.role !== 'admin') {
        return res.status(403).json({ message: "Admin access required" });
      ...
      console.log(`📄 Processing PDF upload: ${req.file.originalname} (${(req.file.size / 1024).toFixed(2)}KB)`);

      // Import extraction function
      const { extractDimensionalProductFromPDF } = await import('./openai');
      
      // Extract product data using AI Vision
      const extracted = await extractDimensionalProductFromPDF(req.file.buffer);

      if (!extracted) {
        return res.status(422).json({ 
          message: "Failed to extract product data from PDF. Please ensure the PDF contains a clear pricing table with dimensions and prices." 
        });
      }

      // Return extracted data for preview/editing
      res.json({
        success: true,
        data: extracted,
        message: `Successfully extracted ${extracted.pricingEntries.length} pricing entries`
      });
  ...
  // Save extracted PDF product to database
  app.post('/api/admin/save-pdf-product', isAuthenticated, async (req: any, res) => {
    try {
      const currentUser = await storage.getUser(req.user?.id);
      if (currentUser?.role !== 'admin') {
        return res.status(403).json({ message: "Admin access required" });
      ...
      const { productData, pricingEntries, defaultDiscount } = req.body;

      if (!productData || !pricingEntries || !Array.isArray(pricingEntries)) {
        return res.status(400).json({ message: "Product data and pricing entries are required" });
      }

      // Validate required fields
      if (!productData.name || !productData.manufacturer) {
        return res.status(400).json({ message: "Product name and manufacturer are required" });
      }
      ...
      const result = await db.transaction(async (tx) => {
        ...
        const productToCreate = {
          ...
          retailPrice: '0', // Will be set based on dimensional pricing
          defaultDiscountType: defaultDiscount?.type || 'percentage' as const,
          defaultDiscountValue: defaultDiscount?.value?.toString() || '0',
          unit: productData.unit || 'each',
          minWidth: productData.minWidth?.toString() || null,
          maxWidth: productData.maxWidth?.toString() || null,
          minLength: productData.minLength?.toString() || null,
          maxLength: productData.maxLength?.toString() || null,
        };

        const [createdProduct] = await tx.insert(products).values(productToCreate).returning();

        // Create all pricing table entries
        const pricingEntriesToCreate = pricingEntries.map((entry: any) => {
          const retailPrice = parseFloat(entry.price);
          let basePrice = retailPrice;

          // Apply manufacturer discount to get base price
          if (defaultDiscount) {
            if (defaultDiscount.type === 'percentage') {
              basePrice = retailPrice * (1 - parseFloat(defaultDiscount.value) / 100);
            } else {
              basePrice = Math.max(0, retailPrice - parseFloat(defaultDiscount.value));
            }
          }

          return {
            productId: createdProduct.id,
            widthMin: entry.widthMin.toString(),
            widthMax: entry.widthMax.toString(),
            lengthMin: entry.lengthMin.toString(),
            lengthMax: entry.lengthMax.toString(),
            retailPrice: retailPrice.toFixed(2),
            basePrice: basePrice.toFixed(2),
          };
        });

        // Bulk insert pricing entries
        const { pricingTables: pricingTablesSchema } = await import('@shared/schema');
        await tx.insert(pricingTablesSchema).values(pricingEntriesToCreate);
      ...
      res.json({
        success: true,
        product: result,
        message: `Successfully created product with ${pricingEntries.length} pricing entries`
      });

    } catch (error) {
      console.error("Save PDF product error:", error);
      res.status(500).json({ 
