...
Rainmaker runs on Vercel. Use a temporary preview only when a separate database
and explicit test credentials have been provisioned.
...
- `server/index.ts` still runs the normal long-lived Express server for local development.
...
- Browser-direct image upload routes use Vercel Blob client uploads.
- Google Workspace staff OAuth is the only human login method.
