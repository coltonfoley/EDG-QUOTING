...
- **Dashboard**: Overview of all quotes with filtering and status indicators, including comprehensive margin/profit tracking
...
- October 16, 2025: Fixed critical bug causing negative prices in quotes - CSV importer was storing manufacturer discounts as 'percentage' type instead of 'dollar' amounts, causing incorrect cost calculations (e.g., $1,371.33 showing as -$8,031.33); migrated 278 products to use correct 'dollar' discount type, added explicit manufacturer field to CSV importer with auto-detection for manufacturer/brand/mfr columns, separated manufacturer from category fields for better data organization
- October 15, 2025: Fixed PDF notes & terms display and added default disclaimer - updated PDF generation to include quote.notes field in contract section, added default value "UNLESS OTHERWISE STATED, PRICING DOES NOT INCLUDE LIFT, FRAMING, FOOTINGS, AND/OR ELECTRICAL" for all new quotes, renamed "Include Contract Terms" toggle to "Include Notes & Terms" in proposal generator, and fixed toggle to enable automatically when notes are present
- October 15, 2025: Fixed product catalog selection to support negative markup values - resolved silent failure when adding products with markdown pricing from catalog to quotes, updated frontend validation to allow negative markup values (±10,000,000), synchronized backend calculation validation to match frontend bounds, and ensured new item form displays after product selection so users can review and save line items with markdown pricing
- October 15, 2025: Enhanced bulk product update to support markdown (negative markup) functionality - enabled negative markup values for both percentage and dollar types to allow price reductions on products with retail pricing, added validation bounds (±10,000,000) with Number.isFinite checks to prevent overflow, implemented floor protection (Math.max(0, total)) to ensure line item totals never go below $0, updated UI to clarify markdown usage
- October 14, 2025: Added comprehensive margin and profit tracking to dashboard - implemented Total Profit, Pipeline Profit, Profit This Month, and Average Margin % metrics with mathematically correct weighted average calculations that account for manufacturer discounts; pipeline overview now displays expected profit for each deal stage alongside deal value
