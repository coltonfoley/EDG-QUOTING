...
- **Key Features**:
    ...
    - **AI-Powered Price Sheet Import**: Drop any manufacturer price sheet (CSV, Excel, PDF) and GPT-4o automatically extracts products with SKU, name, price, cost, category, manufacturer, and confidence scores. Supports chunked processing for large files, editable preview table with SKU column and inline editing, prominent manufacturer confirmation with warning banner, and dedup against existing products by both name and SKU (matches SKU codes embedded in existing product names like `(BH0420033)`). Filename is passed to AI for better manufacturer detection. Available on Admin page under "AI Import" tab (with manual CSV import as fallback). Backend: `POST /api/admin/import-products-ai` (multer file upload → `extractProductsFromPriceSheet` → OpenAI). Frontend: `client/src/components/ai-product-importer.tsx`.
