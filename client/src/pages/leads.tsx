              ...
              : <div className="divide-y">{visible.map((lead) => {
                ...
                return <article key={lead.inquiryId} data-testid={`lead-inquiry-row-${lead.inquiryId}`} className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_auto]">
                  ...
                  <div className="flex min-w-48 flex-col items-stretch gap-2 lg:items-end">
                    ...
                    {lead.leadStatus === "new" && <Button variant="outline" size="sm" disabled={updateStatus.isPending} onClick={() => { setDraftReadyLead(lead); setDraftUrl(""); }} data-testid={`button-mark-draft-ready-${lead.inquiryId}`}><SiGmail aria-hidden="true" className="mr-2 h-4 w-4" />Mark Draft Ready</Button>}
